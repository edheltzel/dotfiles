...
export default tool({
  ...
  async execute() {
    ...
    try {
      const isGitRepo = await Bun.$`git rev-parse --is-inside-work-tree 2>/dev/null`
        .quiet()
        .text()
        .catch(() => "false")
      ...
      if (isGitRepo.trim() === "true") {
        ...
        try {
          const repoRoot = (await Bun.$`git rev-parse --show-toplevel`.quiet().text()).trim()
        ...
        try {
          gitBranch = (await Bun.$`git branch --show-current`.quiet().text()).trim()
          if (!gitBranch) {
            gitBranch = (await Bun.$`git rev-parse --abbrev-ref HEAD`.quiet().text()).trim()
          }
        } catch (e) {
